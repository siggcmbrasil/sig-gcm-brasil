...
export default function TelefonesPage() {
  ...
  async function carregar() {
    ...
    const { data, error } = await supabase
      ...
      .eq("categoria", "TELEFONE_UTIL")
      .eq("ativo", true)
      .order("nome");

    if (error) {
      console.error(error);
      alert("Erro ao carregar telefones úteis.");
      setCarregando(false);
      return;
    }
  ...
  return (
    <div className="p-4 md:p-6 pb-24 space-y-6">
      <SigPageHeader
        ...
        subtitulo="SAMU, Polícia Militar, Bombeiros, hospitais e serviços essenciais."
        icone={Phone}
      />

      {carregando ? (
        <SigCard>
          <p className="text-slate-400">Carregando telefones...</p>
        </SigCard>
      ) : contatos.length === 0 ? (
        <SigEmpty
          titulo="Nenhum telefone cadastrado"
          descricao="Cadastre contatos com categoria Telefone Útil para aparecerem aqui."
        />
      ) : (
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-4 space-y-1 text-sm">
                    <p className="text-slate-300">
                      Fixo: {contato.telefone_fixo || "Não informado"}
                    </p>

                    <p className="text-slate-300">
                      Celular: {contato.celular || "Não informado"}
                    </p>
