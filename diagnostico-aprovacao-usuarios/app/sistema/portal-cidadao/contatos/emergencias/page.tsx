...
export default function EmergenciaPage() {
  ...
  async function carregar() {
    ...
    const { data, error } = await supabase
      ...
      .eq("categoria", "EMERGENCIA")
      .eq("ativo", true)
      .order("nome");

    if (error) {
      console.error(error);
      alert("Erro ao carregar contatos de emergência.");
      setCarregando(false);
      return;
    }
  ...
  return (
    <div className="p-4 md:p-6 pb-24 space-y-6">
      <SigPageHeader
        titulo="Emergências"
        subtitulo="Contatos de resposta rápida para situações urgentes."
        icone={Ambulance}
      />

      {carregando ? (
        <SigCard>
          <p className="text-slate-400">Carregando contatos...</p>
        </SigCard>
      ) : contatos.length === 0 ? (
        <SigEmpty
          titulo="Nenhum contato cadastrado"
          descricao="Cadastre contatos com categoria Emergência para aparecerem aqui."
        />
      ) : (
