...
export default function OrgaosPage() {
  ...
  async function carregar() {
    ...
    const { data, error } = await supabase
      ...
      .eq("categoria", "ORGAO_PUBLICO")
      .eq("ativo", true)
      .order("nome");

    if (error) {
      console.error(error);
      alert("Erro ao carregar órgãos públicos.");
      setCarregando(false);
      return;
    }
  ...
  return (
    <div className="p-4 md:p-6 pb-24 space-y-6">
      <SigPageHeader
        ...
        subtitulo="Prefeitura, secretarias, departamentos e repartições municipais."
        icone={Building2}
      />

      {carregando ? (
        <SigCard>
          <p className="text-slate-400">Carregando órgãos públicos...</p>
        </SigCard>
      ) : contatos.length === 0 ? (
        <SigEmpty
          titulo="Nenhum órgão cadastrado"
          descricao="Cadastre contatos com categoria Órgão Público para aparecerem aqui."
        />
      ) : (
